...
function renderFavorites() {
  ...
  if (!favorites.length) {
    elements.favoritesList.append(createEmptyState("No favorites saved. Use the heart in History to keep one."));
    return;
  }
...
function renderShortcutHints() {
  ...
  elements.favoritesShortcutHelp.textContent =
    `Up to 10 favorites are stored locally. Press ${shortcutModifierLabel} plus a number key to load and speak one.`;
}
